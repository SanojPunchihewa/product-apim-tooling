...
const (
	defaultTaskListTableFormat = "table {{.Name}}\t{{.Type}}\t{{.TriggerCount}}\t{{TriggerInterval}}\t{{TriggerCron}}"
	defaultTaskDetailedFormat  = "detail Name - {{.Name}}\n" +
		"Trigger Type - {{.Type}}\n" +
		"{{if eq .Type cron}}" +
		"Cron Expression - {{.TriggerCron}}\n" +
		"{{else}}" +
...
func PrintTaskList(taskList *artifactutils.TaskList, format string) {
	...
	if taskList.Count > 0 {
		...
		taskListTableHeaders := map[string]string{
			"Name":           nameHeader,
			"TriggerType":    typeHeader,
			"Count":          countHeader,
			"Interval":       intervalHeader,
