...
func TestLoginToMI(t *testing.T) {
	response, _ := base.Execute(t, "mi", "login", miClient.GetEnvName(), "-u", adminUserName, "-p", adminPassword)
	base.Log(response)
...
func TestLoginToMIWithInvalidCredentials(t *testing.T) {
	response, _ := base.Execute(t, "mi", "login", miClient.GetEnvName(), "-u", adminUserName, "-p", "abc123")
	base.Log(response)
...
func TestLoginToMIInUndefinedEnv(t *testing.T) {
	response, _ := base.Execute(t, "mi", "login", undefinedEnv, "-u", adminUserName, "-p", adminPassword)
	base.Log(response)
